...
- `writing-scripts` (myai) — ALWAYS load when creating standalone scripts. NEVER write single-file CLI tools without this. Has Python and TypeScript/Node ecosystem examples.
- `setting-up-logging` — DO load when adding or changing logging. DON'T configure logging manually.
- `building-backends` (myai) — DO load when shaping backend/service/API/workers architecture. DON'T mix transport, domain, and infrastructure blindly. Has Python and TypeScript/Node ecosystem examples.
